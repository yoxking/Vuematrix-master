  ...
  <a-card>
    ...
    <div>
      <div class="operator">
        <a-button
          @click="handleAdd"
          v-hasPermit="['system:taskinfo:addnew']"
          type="primary"
        >新增</a-button>
        <a-button
          @click="handleDel"
          v-hasPermit="['system:taskinfo:delete']"
        >删除</a-button>
        <a-dropdown>
          <a-menu
            @click="handleMenu"
          ...
          >
            <a-menu-item
              key="audit"
              v-hasPermit="['system:taskinfo:audit']"
            >审批</a-menu-item>
            <a-menu-item
              key="import"
              v-hasPermit="['system:taskinfo:import']"
            >导入</a-menu-item>
            <a-menu-item
              key="export"
              v-hasPermit="['system:taskinfo:export']"
            >导出</a-menu-item>
          </a-menu>
          <a-button>
            更多操作
      ...
      >
        ...
        >
          <template v-slot="{ row }">
            <vxe-switch
              ...
              on-label="运行"
              :on-value="1"
              v-hasPermit="['system:taskinfo:change']"
              off-label="暂停"
              :off-value="0"
              size="small"
              @change="changeStatus(row.taskNo,row.taskStatus)"
            ></vxe-switch>
          </template>
        </vxe-table-column>
        <vxe-table-column title="操作">
          <template v-slot="{ row }">
            <vxe-button
              type="text"
              v-hasPermit="['system:taskinfo:start']"
              @click="startTask(row.taskNo)"
            >执行一次</vxe-button>
            <vxe-button
              type="text"
              v-hasPermit="['system:taskinfo:update']"
              @click="handleEdt(row.taskNo)"
            >编辑</vxe-button>
            <vxe-button
              type="text"
              v-hasPermit="['system:taskinfo:detail']"
