<template>
  <a-card>
    ...
    <div>
      <div class="operator">
        <a-button @click="handleAdd" v-hasPermit="['collect:blogclass:addnew']" type="primary">新增</a-button>
        <a-button @click="handleDel" v-hasPermit="['collect:blogclass:delete']">删除</a-button>
        <a-dropdown>
          <a-menu @click="handleMenu"
                  slot="overlay">
            <a-menu-item key="audit" v-hasPermit="['collect:blogclass:audit']">审批</a-menu-item>
            <a-menu-item key="import" v-hasPermit="['collect:blogclass:import']">导入</a-menu-item>
            <a-menu-item key="export" v-hasPermit="['collect:blogclass:export']">导出</a-menu-item>
          </a-menu>
          <a-button>
            更多操作
            <a-icon type="down" />
      ...
      <vxe-table ref="myTable"
        ...
        <vxe-table-column title="操作">
          <template v-slot="{ row }">
            <vxe-button type="text" @click="handleEdt(row.classNo)" v-hasPermit="['collect:blogclass:update']">编辑</vxe-button>
            <vxe-button type="text" @click="handleDet(row.classNo)" v-hasPermit="['collect:blogclass:detail']">详细</vxe-button>
          </template>
        </vxe-table-column>
      </vxe-table>
      <vxe-pager border
...
<script>
import { listBlogclass, delBlogclass, exptBlogclass } from '@/api/collect/blogclass'
  ...
  methods: {
      ...
      if (selectedRecords.length > 0) {
        this.$confirm({
          ...
          onOk () {
            ...
            delBlogclass(selectedRowKeys).then(response => {
              that.getDataSource()
            })
          }
        })
    ...
    handleMenu (e) {
      ...
      if (e.key === 'audit') {
        ...
        exptBlogclass(this.pageParam).then(response => {
    ...
    getDataSource () {
      const that = this
      this.loading = true
      listBlogclass(this.pageParam).then(response => {
