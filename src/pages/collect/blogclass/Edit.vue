...
<script>
import { getBlogclass, treeBlogclass, addBlogclass, uptBlogclass } from '@/api/collect/blogclass'
...
export default {
      ...
      this.$refs.ruleForm.validate(valid => {
        if (valid) {
          if (that.form.classNo === '0') {
            addBlogclass(that.form).then(response => {
              that.$layer.msg(response.msg)
              that.$parent.getDataSource()
              that.$layer.close(that.layerid)
            })
          } else {
            uptBlogclass(that.form).then(response => {
              that.$layer.msg(response.msg)
              that.$parent.getDataSource()
              that.$layer.close(that.layerid)
            })
  ...
  mounted () {
    const that = this
    if (this.id !== '') {
      getBlogclass(this.id).then(response => {
