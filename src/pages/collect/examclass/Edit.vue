...
<script>
import { getExamclass, treeExamclass, addExamclass, uptExamclass } from '@/api/collect/examclass'
      ...
      this.$refs.ruleForm.validate(valid => {
        if (valid) {
          if (that.form.classNo === '0') {
            addExamclass(that.form).then(response => {
              that.$layer.msg(response.msg)
              that.$parent.getDataSource()
              that.$layer.close(that.layerid)
            })
          } else {
            uptExamclass(that.form).then(response => {
              that.$layer.msg(response.msg)
              that.$parent.getDataSource()
              that.$layer.close(that.layerid)
            })
