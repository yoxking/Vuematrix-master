    ...
    <div>
      <div class="operator">
        <a-button @click="handleAdd"
                  v-hasPermit="['collect:salonsinfo:addnew']"
                  type="primary">新增</a-button>
        <a-button @click="handleDel"
                  v-hasPermit="['collect:salonsinfo:delete']">删除</a-button>
        <a-dropdown>
          <a-menu @click="handleMenu"
                  slot="overlay">
            <a-menu-item key="audit"
                         v-hasPermit="['collect:salonsinfo:audit']">审批</a-menu-item>
            <a-menu-item key="import"
                         v-hasPermit="['collect:salonsinfo:import']">导入</a-menu-item>
            <a-menu-item key="export"
                         v-hasPermit="['collect:salonsinfo:export']">导出</a-menu-item>
          </a-menu>
          <a-button>
            更多操作
            <a-icon type="down" />
      ...
      <vxe-table ref="myTable"
        ...
        <vxe-table-column type="seq"
                          title="序号"
                          width="60"></vxe-table-column>
        <vxe-table-column field="salonNo"
                          title="编号"
                          width="120"
                          show-overflow="tooltip"></vxe-table-column>
        <vxe-table-column field="salonName"
                          title="分支名称"></vxe-table-column>
        <vxe-table-column field="classNo"
                          title="类型"></vxe-table-column>
        <vxe-table-column field="startTime"
                          title="时间"
                          show-overflow="tooltip"></vxe-table-column>
        <vxe-table-column title="操作">
          <template v-slot="{ row }">
            <vxe-button type="text"
                        @click="handleEdt(row.salonNo)"
                        v-hasPermit="['collect:salonsinfo:update']">编辑</vxe-button>
            <vxe-button type="text"
                        @click="handleDet(row.salonNo)"
                        v-hasPermit="['collect:salonsinfo:detail']">详细</vxe-button>
          </template>
        </vxe-table-column>
      </vxe-table>
      <vxe-pager border
...
<script>
import { listSalonsinfo, delSalonsinfo, exptSalonsinfo } from '@/api/collect/salonsinfo'
...
export default {
  ...
  methods: {
    ...
    doQuery () {
      ...
      if (this.queryParam.salonName !== '') {
        this.pageParam.condition = " salon_name like '%" + this.queryParam.salonName + "%'"
      } else {
        ...
        this.$confirm({
          ...
          onOk () {
            ...
            selectedRecords.map(function (item) {
              selectedRowKeys.push(item.salonNo)
            })
            delSalonsinfo(selectedRowKeys).then(response => {
              that.getDataSource()
            })
          }
        })
    ...
    handleMenu (e) {
      ...
      if (e.key === 'audit') {
        ...
        exptSalonsinfo(this.pageParam).then(response => {
    ...
    getDataSource () {
      const that = this
      this.loading = true
      listSalonsinfo(this.pageParam).then(response => {
