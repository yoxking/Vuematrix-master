...
export default {
  ...
  methods: {
    ...
    doQuery () {
      ...
      if (this.queryParam.paperTitle !== '') {
        this.pageParam.condition = " paper_title like '%" + this.queryParam.paperTitle + "%'"
      } else {
      ...
      if (selectedRecords.length > 0) {
        this.$confirm({
          ...
          onOk () {
            ...
            delPaperinfo(selectedRowKeys).then(response => {
              that.getDataSource()
            })
          }
        })
    ...
    handleMenu (e) {
      ...
      if (e.key === 'audit') {
        ...
        exptPaperinfo(this.pageParam).then(response => {
    ...
    getDataSource () {
      const that = this
      this.loading = true
      listPaperinfo(this.pageParam).then(response => {
