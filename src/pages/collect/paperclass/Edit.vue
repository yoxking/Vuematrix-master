...
<script>
import { getPaperclass, treePaperclass, addPaperclass, uptPaperclass } from '@/api/collect/paperclass'
      ...
      this.$refs.ruleForm.validate(valid => {
        if (valid) {
          if (that.form.classNo === '0') {
            addPaperclass(that.form).then(response => {
              that.$layer.msg(response.msg)
              that.$parent.getDataSource()
              that.$layer.close(that.layerid)
            })
          } else {
            uptPaperclass(that.form).then(response => {
              that.$layer.msg(response.msg)
              that.$parent.getDataSource()
              that.$layer.close(that.layerid)
            })
