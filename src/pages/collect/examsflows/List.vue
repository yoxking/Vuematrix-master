  ...
  <a-card>
    ...
    <div>
      <div class="operator">
        <a-button
          @click="handleDel"
          v-hasPermit="['collect:examsflows:delete']"
        >删除</a-button>
        <a-dropdown>
          <a-menu
            @click="handleMenu"
          ...
          >
            <a-menu-item
              key="audit"
              v-hasPermit="['collect:examsflows:audit']"
            >审批</a-menu-item>
            <a-menu-item
              key="import"
              v-hasPermit="['collect:examsflows:import']"
            >导入</a-menu-item>
            <a-menu-item
              key="export"
              v-hasPermit="['collect:examsflows:export']"
            >导出</a-menu-item>
          </a-menu>
          <a-button>
            更多操作
        ...
        <vxe-table-column title="操作">
          <template v-slot="{ row }">
            <vxe-button
              ...
              v-hasPermit="['collect:examsflows:detail']"
            >详细</vxe-button>
          </template>
        </vxe-table-column>
      </vxe-table>
...
<script>
import { listExamsflows, delExamsflows, exptExamsflows } from '@/api/collect/examsflows'
  ...
  methods: {
      ...
      if (selectedRecords.length > 0) {
        this.$confirm({
          ...
          onOk () {
            ...
            delExamsflows(selectedRowKeys).then(response => {
              that.getDataSource()
            })
          }
        })
    ...
    handleMenu (e) {
      ...
      if (e.key === 'audit') {
        ...
        exptExamsflows(this.pageParam).then(response => {
    ...
    getDataSource () {
      const that = this
      this.loading = true
      listExamsflows(this.pageParam).then(response => {
