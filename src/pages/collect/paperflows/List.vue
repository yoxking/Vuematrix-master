<template>
  <a-card>
    ...
    <div>
      <div class="operator">
        <a-button @click="handleDel" v-hasPermit="['collect:paperflows:delete']">删除</a-button>
        <a-dropdown>
          <a-menu @click="handleMenu"
                  slot="overlay">
            <a-menu-item key="audit" v-hasPermit="['collect:paperflows:audit']">审批</a-menu-item>
            <a-menu-item key="import" v-hasPermit="['collect:paperflows:import']">导入</a-menu-item>
            <a-menu-item key="export" v-hasPermit="['collect:paperflows:export']">导出</a-menu-item>
          </a-menu>
          <a-button>
            更多操作
            <a-icon type="down" />
      ...
      <vxe-table ref="myTable"
        ...
        <vxe-table-column title="操作">
          <template v-slot="{ row }">
            <vxe-button type="text" @click="handleDet(row.pflowNo)" v-hasPermit="['collect:paperflows:detail']">详细</vxe-button>
          </template>
        </vxe-table-column>
      </vxe-table>
      <vxe-pager border
...
<script>
import { listPaperflows, delPaperflows, exptPaperflows } from '@/api/collect/paperflows'
...
export default {
  ...
  methods: {
    ...
    doQuery () {
      ...
      if (this.queryParam.paperNo !== '') {
        this.pageParam.condition = " paper_no like '%" + this.queryParam.paperNo + "%'"
      } else {
        ...
        this.$confirm({
          ...
          onOk () {
            ...
            selectedRecords.map(function (item) {
              selectedRowKeys.push(item.pflowNo)
            })
            delPaperflows(selectedRowKeys).then(response => {
              that.getDataSource()
            })
          }
        })
    ...
    handleMenu (e) {
      ...
      if (e.key === 'audit') {
        ...
        exptPaperflows(this.pageParam).then(response => {
    ...
    getDataSource () {
      const that = this
      this.loading = true
      listPaperflows(this.pageParam).then(response => {
