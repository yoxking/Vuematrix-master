...
<script>
import { listRemarkinfo, delRemarkinfo, exptRemarkinfo } from '@/api/collect/remarkinfo'
...
export default {
  ...
  methods: {
    ...
    doQuery () {
      ...
      if (this.queryParam.remarkTitle !== '') {
        this.pageParam.condition = " remark_title like '%" + this.queryParam.remarkTitle + "%'"
      } else {
        ...
        this.$confirm({
          ...
          onOk () {
            ...
            selectedRecords.map(function (item) {
              selectedRowKeys.push(item.remarkNo)
            })
            delRemarkinfo(selectedRowKeys).then(response => {
              that.getDataSource()
            })
          }
        })
    ...
    handleMenu (e) {
      ...
      if (e.key === 'audit') {
        ...
        exptRemarkinfo(this.pageParam).then(response => {
    ...
    getDataSource () {
      const that = this
      this.loading = true
      listRemarkinfo(this.pageParam).then(response => {
