...
<script>
import { getSalonclass, treeSalonclass, addSalonclass, uptSalonclass } from '@/api/collect/salonclass'
...
export default {
      ...
      this.$refs.ruleForm.validate(valid => {
        if (valid) {
          if (that.form.classNo === '0') {
            addSalonclass(that.form).then(response => {
              that.$layer.msg(response.msg)
              that.$parent.getDataSource()
              that.$layer.close(that.layerid)
            })
          } else {
            uptSalonclass(that.form).then(response => {
              that.$layer.msg(response.msg)
              that.$parent.getDataSource()
              that.$layer.close(that.layerid)
            })
  ...
  mounted () {
    const that = this
    if (this.id !== '') {
      getSalonclass(this.id).then(response => {
