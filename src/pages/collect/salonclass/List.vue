<template>
  <a-card>
    ...
    <div>
      <div class="operator">
        <a-button @click="handleAdd" v-hasPermit="['collect:salonclass:addnew']" type="primary">新增</a-button>
        <a-button @click="handleDel" v-hasPermit="['collect:salonclass:delete']">删除</a-button>
        <a-dropdown>
          <a-menu @click="handleMenu"
                  slot="overlay">
            <a-menu-item key="audit" v-hasPermit="['collect:salonclass:audit']">审批</a-menu-item>
            <a-menu-item key="import" v-hasPermit="['collect:salonclass:import']">导入</a-menu-item>
            <a-menu-item key="export" v-hasPermit="['collect:salonclass:export']">导出</a-menu-item>
          </a-menu>
          <a-button>
            更多操作
            <a-icon type="down" />
      ...
      <vxe-table ref="myTable"
        ...
        <vxe-table-column title="操作">
          <template v-slot="{ row }">
            <vxe-button type="text" @click="handleEdt(row.classNo)" v-hasPermit="['collect:salonclass:update']">编辑</vxe-button>
            <vxe-button type="text" @click="handleDet(row.classNo)" v-hasPermit="['collect:salonclass:detail']">详细</vxe-button>
          </template>
        </vxe-table-column>
      </vxe-table>
      <vxe-pager border
...
<script>
import { listSalonclass, delSalonclass, exptSalonclass } from '@/api/collect/salonclass'
  ...
  methods: {
      ...
      if (selectedRecords.length > 0) {
        this.$confirm({
          ...
          onOk () {
            ...
            delSalonclass(selectedRowKeys).then(response => {
              that.getDataSource()
            })
          }
        })
    ...
    handleMenu (e) {
      ...
      if (e.key === 'audit') {
        ...
        exptSalonclass(this.pageParam).then(response => {
    ...
    getDataSource () {
      const that = this
      this.loading = true
      listSalonclass(this.pageParam).then(response => {
