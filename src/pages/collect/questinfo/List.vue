<template>
  <a-card>
    ...
    <div>
      <div class="operator">
        <a-button @click="handleAdd" v-hasPermit="['collect:questinfo:addnew']" type="primary">新增</a-button>
        <a-button @click="handleDel" v-hasPermit="['collect:questinfo:delete']">删除</a-button>
        <a-dropdown>
          <a-menu @click="handleMenu"
                  slot="overlay">
            <a-menu-item key="audit" v-hasPermit="['collect:questinfo:audit']">审批</a-menu-item>
            <a-menu-item key="import" v-hasPermit="['collect:questinfo:import']">导入</a-menu-item>
            <a-menu-item key="export" v-hasPermit="['collect:questinfo:export']">导出</a-menu-item>
          </a-menu>
          <a-button>
            更多操作
            <a-icon type="down" />
      ...
      <vxe-table ref="myTable"
        ...
        <vxe-table-column type="seq"
                          title="序号"
                          width="60"></vxe-table-column>
        <vxe-table-column field="questNo"
                          title="编号" width="120" show-overflow="tooltip"></vxe-table-column>
        <vxe-table-column field="questTitle"
        ...
        <vxe-table-column title="操作">
          <template v-slot="{ row }">
            <vxe-button type="text" @click="handleEdt(row.questNo)" v-hasPermit="['collect:questinfo:update']">编辑</vxe-button>
            <vxe-button type="text" @click="handleDet(row.questNo)" v-hasPermit="['collect:questinfo:detail']">详细</vxe-button>
          </template>
        </vxe-table-column>
      </vxe-table>
      <vxe-pager border
...
<script>
import { listQuestinfo, delQuestinfo, exptQuestinfo } from '@/api/collect/questinfo'
...
export default {
  ...
  methods: {
    ...
    doQuery () {
      ...
      if (this.queryParam.questTitle !== '') {
        this.pageParam.condition = " quest_title like '%" + this.queryParam.questTitle + "%'"
      } else {
      ...
      if (selectedRecords.length > 0) {
        this.$confirm({
          ...
          onOk () {
            ...
            delQuestinfo(selectedRowKeys).then(response => {
              that.getDataSource()
            })
          }
        })
    ...
    handleMenu (e) {
      ...
      if (e.key === 'audit') {
        ...
        exptQuestinfo(this.pageParam).then(response => {
