...
<script>
import { listExamsinfo, delExamsinfo, exptExamsinfo } from '@/api/collect/examsinfo'
  ...
  methods: {
    ...
    handleMenu (e) {
      ...
      if (e.key === 'audit') {
        ...
        exptExamsinfo(this.pageParam).then(response => {
    ...
    getDataSource () {
      const that = this
      this.loading = true
      listExamsinfo(this.pageParam).then(response => {
