<template>
  <a-card>
    ...
    <div>
      <div class="operator">
        <a-button @click="handleAdd" v-hasPermit="['system:flowbutton:addnew']" type="primary">新建</a-button>
        <a-button @click="handleDel" v-hasPermit="['system:flowbutton:delete']">删除</a-button>
        <a-dropdown>
          <a-menu @click="handleMenu"
                  slot="overlay">
            <a-menu-item key="audit" v-hasPermit="['system:flowbutton:audit']">审批</a-menu-item>
            <a-menu-item key="import" v-hasPermit="['system:flowbutton:import']">导入</a-menu-item>
            <a-menu-item key="export" v-hasPermit="['system:flowbutton:export']">导出</a-menu-item>
          </a-menu>
          <a-button>
            更多操作
            <a-icon type="down" />
      ...
      <vxe-table ref="myTable"
        ...
        <vxe-table-column type="seq"
                          title="序号"
                          width="60"></vxe-table-column>
        <vxe-table-column field="btnNo"
                          title="编号" width="120" show-overflow="tooltip"></vxe-table-column>
        <vxe-table-column field="btnTitle"
                          title="按钮名称"></vxe-table-column>
        <vxe-table-column field="btnIcon"
                          title="按钮图标"></vxe-table-column>
        <vxe-table-column field="checkState"
                          title="状态"
                          show-overflow="tooltip"></vxe-table-column>
        <vxe-table-column title="操作">
          <template v-slot="{ row }">
            <vxe-button type="text" @click="handleEdt(row.btnNo)" v-hasPermit="['system:flowbutton:update']">编辑</vxe-button>
            <vxe-button type="text" @click="handleDet(row.btnNo)" v-hasPermit="['system:flowbutton:detail']">详细</vxe-button>
          </template>
        </vxe-table-column>
      </vxe-table>
      <vxe-pager border
...
export default {
  ...
  methods: {
    ...
    doQuery () {
      ...
      if (this.queryParam.btnTitle !== '') {
        this.pageParam.condition = " btn_title like '%" + this.queryParam.btnTitle + "%'"
      } else {
        ...
        this.$confirm({
          ...
          onOk () {
            ...
            selectedRecords.map(function (item) {
              selectedRowKeys.push(item.btnNo)
            })
