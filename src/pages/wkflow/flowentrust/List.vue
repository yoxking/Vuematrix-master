  ...
  <a-card>
    ...
    <div>
      <div class="operator">
        <a-button
          @click="handleAdd"
          v-hasPermit="['wkflow:flowentrust:addnew']"
          type="primary"
        >新增</a-button>
        <a-button
          @click="handleDel"
          v-hasPermit="['wkflow:flowentrust:delete']"
        >删除</a-button>
        <a-dropdown>
          <a-menu
            @click="handleMenu"
          ...
          >
            <a-menu-item
              key="audit"
              v-hasPermit="['wkflow:flowentrust:audit']"
            >审批</a-menu-item>
            <a-menu-item
              key="import"
              v-hasPermit="['wkflow:flowentrust:import']"
            >导入</a-menu-item>
            <a-menu-item
              key="export"
              v-hasPermit="['wkflow:flowentrust:export']"
            >导出</a-menu-item>
          </a-menu>
          <a-button>
            更多操作
      ...
      >
        ...
        <vxe-table-column title="操作">
          <template v-slot="{ row }">
            <vxe-button
              type="text"
              @click="handleEdt(row.entrustNo)"
              v-hasPermit="['wkflow:flowentrust:update']"
            >编辑</vxe-button>
            <vxe-button
              type="text"
              @click="handleDet(row.entrustNo)"
              v-hasPermit="['wkflow:flowentrust:detail']"
            >详细</vxe-button>
          </template>
        </vxe-table-column>
      </vxe-table>
...
<script>
import { listFlowentrust, delFlowentrust, exptFlowentrust } from '@/api/wkflow/flowentrust'
  ...
  methods: {
    ...
    handleMenu (e) {
      ...
      if (e.key === 'audit') {
        ...
        exptFlowentrust(this.pageParam).then(response => {
    ...
    getDataSource () {
      const that = this
      this.loading = true
      listFlowentrust(this.pageParam).then(response => {
