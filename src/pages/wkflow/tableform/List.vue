  ...
  <a-card>
    ...
    <div>
      <div class="operator">
        <a-button
          @click="handleAdd"
          v-hasPermit="['wkflow:tableform:addnew']"
          type="primary"
        >新增</a-button>
        <a-button
          @click="handleDel"
          v-hasPermit="['wkflow:tableform:delete']"
        >删除</a-button>
        <a-dropdown>
          <a-menu
            @click="handleMenu"
          ...
          >
            <a-menu-item
              key="audit"
              v-hasPermit="['wkflow:tableform:audit']"
            >审批</a-menu-item>
            <a-menu-item
              key="import"
              v-hasPermit="['wkflow:tableform:import']"
            >导入</a-menu-item>
            <a-menu-item
              key="export"
              v-hasPermit="['wkflow:tableform:export']"
            >导出</a-menu-item>
          </a-menu>
          <a-button>
            更多操作
      ...
      >
        ...
        >
          <template v-slot="{ row }">
            <vxe-button
              type="text"
              v-hasPermit="['wkflow:tableform:dcolumn']"
            >设计字段</vxe-button>
            <vxe-button
              type="text"
              @click="designForm(row.formNo)"
              v-hasPermit="['wkflow:tableform:dform']"
            >设计表单</vxe-button>
            <vxe-button
              type="text"
              @click="handleEdt(row.formNo)"
              v-hasPermit="['wkflow:tableform:update']"
            >编辑</vxe-button>
            <vxe-button
              type="text"
              @click="handleDet(row.formNo)"
              v-hasPermit="['wkflow:tableform:detail']"
            >详细</vxe-button>
          </template>
        </vxe-table-column>
      </vxe-table>
  ...
  methods: {
    ...
    doQuery () {
      ...
      if (this.queryParam.formName !== '') {
        this.pageParam.condition = " form_name like '%" + this.queryParam.formName + "%'"
      } else {
        ...
        this.$confirm({
          ...
          onOk () {
            ...
            selectedRecords.map(function (item) {
              selectedRowKeys.push(item.formNo)
            })
            delTableform(selectedRowKeys).then(response => {
              that.getDataSource()
            })
          }
        })
    ...
    getDataSource () {
      const that = this
      this.loading = true
      listTableform(this.pageParam).then(response => {
