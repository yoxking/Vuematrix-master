...
<script>
import { getFlowtaske, addFlowtaske, uptFlowtaske } from '@/api/wkflow/flowtaske'
...
export default {
      ...
      this.$refs.ruleForm.validate(valid => {
        if (valid) {
          if (that.form.branchNo === '0') {
            addFlowtaske(that.form).then(response => {
              that.$message.success(response.msg)
              that.$emit('close', { code: response.code })
            })
          } else {
            uptFlowtaske(that.form).then(response => {
              that.$message.success(response.msg)
              that.$emit('close', { code: response.code })
            })
          }
  ...
  mounted () {
    if (this.id !== '') {
      const that = this
      getFlowtaske(this.id).then(response => {
