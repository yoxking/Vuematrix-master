<template>
  <a-card>
    ...
    <div>
      <div class="operator">
        <a-button @click="handleAdd" v-hasPermit="['wkflow:flowtaske:addnew']" type="primary">新增</a-button>
        <a-button @click="handleDel" v-hasPermit="['wkflow:flowtaske:delete']">删除</a-button>
        <a-dropdown>
          <a-menu @click="handleMenu"
                  slot="overlay">
            <a-menu-item key="audit" v-hasPermit="['wkflow:flowtaske:audit']">审批</a-menu-item>
            <a-menu-item key="import" v-hasPermit="['wkflow:flowtaske:import']">导入</a-menu-item>
            <a-menu-item key="export" v-hasPermit="['wkflow:flowtaske:export']">导出</a-menu-item>
          </a-menu>
          <a-button>
            更多操作
            <a-icon type="down" />
      ...
      <vxe-table ref="myTable"
        ...
        <vxe-table-column field="taskTitle"
                          title="任务名称"></vxe-table-column>
        <vxe-table-column field="taskType"
        ...
        <vxe-table-column title="操作">
          <template v-slot="{ row }">
            <vxe-button type="text" @click="handleEdt(row.taskNo)" v-hasPermit="['wkflow:flowtaske:update']">编辑</vxe-button>
            <vxe-button type="text" @click="handleDet(row.taskNo)" v-hasPermit="['wkflow:flowtaske:detail']">详细</vxe-button>
          </template>
        </vxe-table-column>
      </vxe-table>
      <vxe-pager border
...
<script>
import { listFlowtaske, delFlowtaske, exptFlowtaske } from '@/api/wkflow/flowtaske'
  ...
  methods: {
      ...
      if (selectedRecords.length > 0) {
        this.$confirm({
          ...
          onOk () {
            ...
            delFlowtaske(selectedRowKeys).then(response => {
              that.getDataSource()
            })
          }
        })
    ...
    handleMenu (e) {
      ...
      if (e.key === 'audit') {
        ...
        exptFlowtaske(this.pageParam).then(response => {
    ...
    getDataSource () {
      const that = this
      this.loading = true
      listFlowtaske(this.pageParam).then(response => {
