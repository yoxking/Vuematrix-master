  ...
  <a-card>
    ...
    <div>
      <div class="operator">
        <a-button
          @click="handleAdd"
          v-hasPermit="['process:goodsinfo:addnew']"
          type="primary"
        >新增</a-button>
        <a-button
          @click="handleDel"
          v-hasPermit="['process:goodsinfo:delete']"
        >删除</a-button>
        <a-dropdown>
          <a-menu
            @click="handleMenu"
          ...
          >
            <a-menu-item
              key="audit"
              v-hasPermit="['process:goodsinfo:audit']"
            >审批</a-menu-item>
            <a-menu-item
              key="import"
              v-hasPermit="['process:goodsinfo:import']"
            >导入</a-menu-item>
            <a-menu-item
              key="export"
              v-hasPermit="['process:goodsinfo:export']"
            >导出</a-menu-item>
          </a-menu>
          <a-button>
            更多操作
      ...
      >
        ...
        <vxe-table-column title="操作">
          <template v-slot="{ row }">
            <vxe-button
              type="text"
              @click="handleEdt(row.goodsNo)"
              v-hasPermit="['process:goodsinfo:update']"
            >编辑</vxe-button>
            <vxe-button
              type="text"
              @click="handleDet(row.goodsNo)"
              v-hasPermit="['process:goodsinfo:detail']"
            >详细</vxe-button>
          </template>
        </vxe-table-column>
      </vxe-table>
...
<script>
import { listGoodsinfo, delGoodsinfo, exptGoodsinfo } from '@/api/process/goodsinfo'
...
export default {
  ...
  methods: {
    ...
    doQuery () {
      ...
      if (this.queryParam.goodsCname !== '') {
        this.pageParam.condition = " goods_cname like '%" + this.queryParam.goodsCname + "%'"
      } else {
        ...
        this.$confirm({
          ...
          onOk () {
            ...
            selectedRecords.map(function (item) {
              selectedRowKeys.push(item.goodsNo)
            })
            delGoodsinfo(selectedRowKeys).then(response => {
              that.getDataSource()
            })
          }
        })
    ...
    handleMenu (e) {
      ...
      if (e.key === 'audit') {
        ...
        exptGoodsinfo(this.pageParam).then(response => {
