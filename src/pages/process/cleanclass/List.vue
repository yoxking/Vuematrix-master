  ...
  <a-card>
    ...
    <div>
      <div class="operator">
        <a-button
          @click="handleAdd"
          v-hasPermit="['process:cleanclass:addnew']"
          type="primary"
        >新增</a-button>
        <a-button
          @click="handleDel"
          v-hasPermit="['process:cleanclass:delete']"
        >删除</a-button>
        <a-dropdown>
          <a-menu
            @click="handleMenu"
          ...
          >
            <a-menu-item
              key="audit"
              v-hasPermit="['process:cleanclass:audit']"
            >审批</a-menu-item>
            <a-menu-item
              key="import"
              v-hasPermit="['process:cleanclass:import']"
            >导入</a-menu-item>
            <a-menu-item
              key="export"
              v-hasPermit="['process:cleanclass:export']"
            >导出</a-menu-item>
          </a-menu>
          <a-button>
            更多操作
        ...
        <vxe-table-column title="操作">
          <template v-slot="{ row }">
            <vxe-button
              ...
              v-hasPermit="['process:cleanclass:update']"
            >编辑</vxe-button>
            <vxe-button
              type="text"
              @click="handleDet(row.classNo)"
              v-hasPermit="['process:cleanclass:detail']"
            >详细</vxe-button>
          </template>
        </vxe-table-column>
      </vxe-table>
...
<script>
import { listCleanclass, delCleanclass, exptCleanclass } from '@/api/process/cleanclass'
  ...
  methods: {
      ...
      if (selectedRecords.length > 0) {
        this.$confirm({
          ...
          onOk () {
            ...
            delCleanclass(selectedRowKeys).then(response => {
              that.getDataSource()
            })
          }
        })
    ...
    handleMenu (e) {
      ...
      if (e.key === 'audit') {
        ...
        exptCleanclass(this.pageParam).then(response => {
    ...
    getDataSource () {
      const that = this
      this.loading = true
      listCleanclass(this.pageParam).then(response => {
