  ...
  <a-card>
    ...
    <div>
      <div class="operator">
        <a-button
          @click="handleAdd"
          v-hasPermit="['process:sterytype:addnew']"
          type="primary"
        >新增</a-button>
        <a-button
          @click="handleDel"
          v-hasPermit="['process:sterytype:delete']"
        >删除</a-button>
        <a-dropdown>
          <a-menu
            @click="handleMenu"
          ...
          >
            <a-menu-item
              key="audit"
              v-hasPermit="['process:sterytype:audit']"
            >审批</a-menu-item>
            <a-menu-item
              key="import"
              v-hasPermit="['process:sterytype:import']"
            >导入</a-menu-item>
            <a-menu-item
              key="export"
              v-hasPermit="['process:sterytype:export']"
            >导出</a-menu-item>
          </a-menu>
          <a-button>
            更多操作
      ...
      >
        ...
        <vxe-table-column title="操作">
          <template v-slot="{ row }">
            <vxe-button
              type="text"
              @click="handleEdt(row.typeNo)"
              v-hasPermit="['process:sterytype:update']"
            >编辑</vxe-button>
            <vxe-button
              type="text"
              @click="handleDet(row.typeNo)"
              v-hasPermit="['process:sterytype:detail']"
            >详细</vxe-button>
          </template>
        </vxe-table-column>
      </vxe-table>
...
<script>
import { listSterytype, delSterytype, exptSterytype } from '@/api/process/sterytype'
  ...
  methods: {
    ...
    doQuery () {
      ...
      if (this.queryParam.typeName !== '') {
        this.pageParam.condition = " type_name like '%" + this.queryParam.typeName + "%'"
      } else {
        ...
        this.$confirm({
          ...
          onOk () {
            ...
            selectedRecords.map(function (item) {
              selectedRowKeys.push(item.typeNo)
            })
            delSterytype(selectedRowKeys).then(response => {
              that.getDataSource()
            })
          }
        })
    ...
    handleMenu (e) {
      ...
      if (e.key === 'audit') {
        ...
        exptSterytype(this.pageParam).then(response => {
    ...
    getDataSource () {
      const that = this
      this.loading = true
      listSterytype(this.pageParam).then(response => {
