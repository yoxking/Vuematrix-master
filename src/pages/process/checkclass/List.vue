  ...
  <a-card>
    ...
    <div>
      <div class="operator">
        <a-button
          @click="handleAdd"
          v-hasPermit="['process:checkclass:addnew']"
          type="primary"
        >新增</a-button>
        <a-button
          @click="handleDel"
          v-hasPermit="['process:checkclass:delete']"
        >删除</a-button>
        <a-dropdown>
          <a-menu
            @click="handleMenu"
          ...
          >
            <a-menu-item
              key="audit"
              v-hasPermit="['process:checkclass:audit']"
            >审批</a-menu-item>
            <a-menu-item
              key="import"
              v-hasPermit="['process:checkclass:import']"
            >导入</a-menu-item>
            <a-menu-item
              key="export"
              v-hasPermit="['process:checkclass:export']"
            >导出</a-menu-item>
          </a-menu>
          <a-button>
            更多操作
        ...
        <vxe-table-column title="操作">
          <template v-slot="{ row }">
            <vxe-button
              ...
              v-hasPermit="['process:checkclass:update']"
            >编辑</vxe-button>
            <vxe-button
              type="text"
              @click="handleDet(row.classNo)"
              v-hasPermit="['process:checkclass:detail']"
            >详细</vxe-button>
          </template>
        </vxe-table-column>
      </vxe-table>
...
<script>
import { listCheckclass, delCheckclass, exptCheckclass } from '@/api/process/checkclass'
  ...
  methods: {
      ...
      if (selectedRecords.length > 0) {
        this.$confirm({
          ...
          onOk () {
            ...
            delCheckclass(selectedRowKeys).then(response => {
              that.getDataSource()
            })
          }
        })
    ...
    handleMenu (e) {
      ...
      if (e.key === 'audit') {
        ...
        exptCheckclass(this.pageParam).then(response => {
    ...
    getDataSource () {
      const that = this
      this.loading = true
      listCheckclass(this.pageParam).then(response => {
