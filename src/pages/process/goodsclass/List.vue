  ...
  <a-card>
    ...
    <div>
      <div class="operator">
        <a-button
          @click="handleAdd"
          v-hasPermit="['process:goodsclass:addnew']"
          type="primary"
        >新增</a-button>
        <a-button
          @click="handleDel"
          v-hasPermit="['process:goodsclass:delete']"
        >删除</a-button>
        <a-dropdown>
          <a-menu
            @click="handleMenu"
          ...
          >
            <a-menu-item
              key="audit"
              v-hasPermit="['process:goodsclass:audit']"
            >审批</a-menu-item>
            <a-menu-item
              key="import"
              v-hasPermit="['process:goodsclass:import']"
            >导入</a-menu-item>
            <a-menu-item
              key="export"
              v-hasPermit="['process:goodsclass:export']"
            >导出</a-menu-item>
          </a-menu>
          <a-button>
            更多操作
        ...
        <vxe-table-column title="操作">
          <template v-slot="{ row }">
            <vxe-button
              ...
              v-hasPermit="['process:goodsclass:update']"
            >编辑</vxe-button>
            <vxe-button
              type="text"
              @click="handleDet(row.classNo)"
              v-hasPermit="['process:goodsclass:detail']"
            >详细</vxe-button>
          </template>
        </vxe-table-column>
      </vxe-table>
...
<script>
import { listGoodsclass, delGoodsclass, exptGoodsclass } from '@/api/process/goodsclass'
  ...
  methods: {
      ...
      if (selectedRecords.length > 0) {
        this.$confirm({
          ...
          onOk () {
            ...
            delGoodsclass(selectedRowKeys).then(response => {
              that.getDataSource()
            })
          }
        })
    ...
    handleMenu (e) {
      ...
      if (e.key === 'audit') {
        ...
        exptGoodsclass(this.pageParam).then(response => {
    ...
    getDataSource () {
      const that = this
      this.loading = true
      listGoodsclass(this.pageParam).then(response => {
