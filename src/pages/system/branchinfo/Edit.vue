...
<script>
import { getBranchinfo, addBranchinfo, uptBranchinfo } from '@/api/system/branchinfo'
...
export default {
      ...
      this.$refs.ruleForm.validate(valid => {
        if (valid) {
          if (that.form.branchNo === '0') {
            addBranchinfo(that.form).then(response => {
              that.$layer.msg(response.msg)
              that.$parent.getDataSource()
              that.$layer.close(that.layerid)
            })
          } else {
            uptBranchinfo(that.form).then(response => {
              that.$layer.msg(response.msg)
              that.$parent.getDataSource()
              that.$layer.close(that.layerid)
            })
  ...
  mounted () {
    if (this.id !== '') {
      const that = this
      getBranchinfo(this.id).then(response => {
