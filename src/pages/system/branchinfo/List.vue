    ...
    <div>
      <div class="operator">
        <a-button
          @click="handleAdd"
          type="primary"
          v-hasPermit="['system:branchinfo:addnew']"
        >新增</a-button>
        <a-button
          @click="handleDel"
          v-hasPermit="['system:branchinfo:delete']"
        >删除</a-button>
        <a-dropdown>
          <a-menu
            @click="handleMenu"
          ...
          >
            <a-menu-item
              key="audit"
              v-hasPermit="['system:branchinfo:audit']"
            >审批</a-menu-item>
            <a-menu-item
              key="import"
              v-hasPermit="['system:branchinfo:import']"
            >导入</a-menu-item>
            <a-menu-item
              key="export"
              v-hasPermit="['system:branchinfo:export']"
            >导出</a-menu-item>
          </a-menu>
          <a-button>
            更多操作
        ...
        <vxe-table-column title="操作">
          <template v-slot="{ row }">
            <vxe-button
              ...
              v-hasPermit="['system:branchinfo:update']"
            >编辑</vxe-button>
            <vxe-button
              type="text"
              @click="handleDet(row.branchNo)"
              v-hasPermit="['system:branchinfo:detail']"
            >详细</vxe-button>
          </template>
        </vxe-table-column>
      </vxe-table>
...
<script>
import { listBranchinfo, delBranchinfo, exptBranchinfo } from '@/api/system/branchinfo'
...
export default {
  ...
  methods: {
      ...
      if (selectedRecords.length > 0) {
        this.$confirm({
          ...
          onOk () {
            ...
            delBranchinfo(selectedRowKeys).then(response => {
              that.getDataSource()
            })
          }
        })
    ...
    getDataSource () {
      const that = this
      this.loading = true
      listBranchinfo(this.pageParam).then(response => {
