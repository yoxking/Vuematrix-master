    ...
    <div>
      <div class="operator">
        <a-button @click="handleAdd"
                  v-hasPermit="['system:ruserrota:addnew']"
                  type="primary">新增</a-button>
        <a-button @click="handleDel"
                  v-hasPermit="['system:ruserrota:delete']">删除</a-button>
        <a-dropdown>
          <a-menu @click="handleMenu"
                  slot="overlay">
            <a-menu-item key="audit"
                         v-hasPermit="['system:ruserrota:audit']">审批</a-menu-item>
            <a-menu-item key="import"
                         v-hasPermit="['system:ruserrota:import']">导入</a-menu-item>
            <a-menu-item key="export"
                         v-hasPermit="['system:ruserrota:export']">导出</a-menu-item>
          </a-menu>
          <a-button>
            更多操作
            <a-icon type="down" />
      ...
      <vxe-table ref="myTable"
        ...
        <vxe-table-column type="seq"
                          title="序号"
                          width="60"></vxe-table-column>
        <vxe-table-column field="rotaNo"
                          title="编号"
                          width="120"
                          show-overflow="tooltip"></vxe-table-column>
        <vxe-table-column field="userNo"
                          title="用户"></vxe-table-column>
        <vxe-table-column field="rotaType"
                          title="类型"></vxe-table-column>
        <vxe-table-column field="weekDay1"
                          title="值班表"
                          show-overflow="tooltip"></vxe-table-column>
        <vxe-table-column title="操作">
          <template v-slot="{ row }">
            <vxe-button type="text"
                        @click="handleEdt(row.rotaNo)"
                        v-hasPermit="['system:ruserrota:update']">编辑</vxe-button>
            <vxe-button type="text"
                        @click="handleDet(row.rotaNo)"
                        v-hasPermit="['system:ruserrota:detail']">详细</vxe-button>
          </template>
        </vxe-table-column>
      </vxe-table>
      <vxe-pager border
...
<script>
import { listRuserrota, delRuserrota, exptRuserrota } from '@/api/system/ruserrota'
...
export default {
  ...
  methods: {
    ...
    doQuery () {
      ...
      if (this.queryParam.ruserNo !== '') {
        this.pageParam.condition = " user_no like '%" + this.queryParam.ruserNo + "%'"
      } else {
        ...
        this.$confirm({
          ...
          onOk () {
            ...
            selectedRecords.map(function (item) {
              selectedRowKeys.push(item.rotaNo)
            })
            delRuserrota(selectedRowKeys).then(response => {
              that.getDataSource()
            })
          }
        })
    ...
    handleMenu (e) {
      ...
      if (e.key === 'audit') {
        ...
        exptRuserrota(this.pageParam).then(response => {
    ...
    getDataSource () {
      const that = this
      this.loading = true
      listRuserrota(this.pageParam).then(response => {
