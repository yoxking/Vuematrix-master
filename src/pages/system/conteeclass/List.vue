<template>
  <a-card>
    ...
    <div>
      <div class="operator">
        <a-button @click="handleAdd" v-hasPermit="['system:conteeclass:addnew']" type="primary">新建</a-button>
        <a-button @click="handleDel" v-hasPermit="['system:conteeclass:delete']">删除</a-button>
        <a-dropdown>
          <a-menu @click="handleMenu"
                  slot="overlay">
            <a-menu-item key="audit" v-hasPermit="['system:conteeclass:audit']">审批</a-menu-item>
            <a-menu-item key="import" v-hasPermit="['system:conteeclass:import']">导入</a-menu-item>
            <a-menu-item key="export" v-hasPermit="['system:conteeclass:export']">导出</a-menu-item>
          </a-menu>
          <a-button>
            更多操作
            <a-icon type="down" />
      ...
      <vxe-table ref="myTable"
        ...
        <vxe-table-column title="操作">
          <template v-slot="{ row }">
            <vxe-button type="text" @click="handleEdt(row.classNo)" v-hasPermit="['system:conteeclass:update']">编辑</vxe-button>
            <vxe-button type="text" @click="handleDet(row.classNo)" v-hasPermit="['system:conteeclass:detail']">详细</vxe-button>
          </template>
        </vxe-table-column>
      </vxe-table>
      <vxe-pager border
...
<script>
import { listConteeclass, delConteeclass, exptConteeclass } from '@/api/system/conteeclass'
  ...
  methods: {
      ...
      if (selectedRecords.length > 0) {
        this.$confirm({
          ...
          onOk () {
            ...
            delConteeclass(selectedRowKeys).then(response => {
              that.getDataSource()
            })
          }
        })
    ...
    handleMenu (e) {
      ...
      if (e.key === 'audit') {
        ...
        exptConteeclass(this.pageParam).then(response => {
    ...
    getDataSource () {
      const that = this
      this.loading = true
      listConteeclass(this.pageParam).then(response => {
