...
<script>
import { getOperatelogs, addOperatelogs, uptOperatelogs } from '@/api/system/operatelogs'
...
export default {
      ...
      this.$refs.ruleForm.validate(valid => {
        if (valid) {
          if (that.form.branchNo === '0') {
            addOperatelogs(that.form).then(response => {
              that.$message.success(response.msg)
              that.$emit('close', { code: response.code })
            })
          } else {
            uptOperatelogs(that.form).then(response => {
              that.$message.success(response.msg)
              that.$emit('close', { code: response.code })
            })
          }
  ...
  mounted () {
    if (this.id !== '') {
      const that = this
      getOperatelogs(this.id).then(response => {
