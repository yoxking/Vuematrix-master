<template>
  <a-card>
    ...
    <div>
      <div class="operator">
        <a-button @click="handleDel" v-hasPermit="['system:operatelog:delete']">删除</a-button>
        <a-dropdown>
          <a-menu @click="handleMenu"
                  slot="overlay">
            <a-menu-item key="audit" v-hasPermit="['system:operatelog:audit']">审批</a-menu-item>
            <a-menu-item key="import" v-hasPermit="['system:operatelog:import']">导入</a-menu-item>
            <a-menu-item key="export" v-hasPermit="['system:operatelog:export']">导出</a-menu-item>
          </a-menu>
          <a-button>
            更多操作
            <a-icon type="down" />
        ...
        <vxe-table-column title="操作">
          <template v-slot="{ row }">
            <vxe-button type="text"
                        @click="handleDet(row.oplogNo)" v-hasPermit="['system:operatelog:detail']">详细</vxe-button>
          </template>
        </vxe-table-column>
      </vxe-table>
      <vxe-pager border
...
<script>
import { listOperatelog, delOperatelog, exptOperatelog } from '@/api/system/operatelog'
  ...
  methods: {
      ...
      if (selectedRecords.length > 0) {
        this.$confirm({
          ...
          onOk () {
            ...
            delOperatelog(selectedRowKeys).then(response => {
              that.getDataSource()
            })
          }
        })
    ...
    handleMenu (e) {
      ...
      if (e.key === 'audit') {
        ...
        exptOperatelog(this.pageParam).then(response => {
    ...
    getDataSource () {
      const that = this
      this.loading = true
      listOperatelog(this.pageParam).then(response => {
