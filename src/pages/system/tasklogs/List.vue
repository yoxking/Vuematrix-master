        ...
        <vxe-table-column title="操作">
          <template v-slot="{ row }">
            <vxe-button type="text"
                        @click="handleDet(row.taskLogno)">详细</vxe-button>
          </template>
        </vxe-table-column>
      </vxe-table>
      <vxe-pager border
...
<script>
import { listTasklogs, delTasklogs, exptTasklogs } from '@/api/system/tasklogs'
  ...
  methods: {
      ...
      if (selectedRecords.length > 0) {
        this.$confirm({
          ...
          onOk () {
            ...
            delTasklogs(selectedRowKeys).then(response => {
              that.getDataSource()
            })
          }
        })
    ...
    handleMenu (e) {
      ...
      if (e.key === 'audit') {
        ...
        exptTasklogs(this.pageParam).then(response => {
    ...
    getDataSource () {
      const that = this
      this.loading = true
      listTasklogs(this.pageParam).then(response => {
