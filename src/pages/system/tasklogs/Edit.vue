...
<script>
import { getTasklogs, addTasklogs, uptTasklogs } from '@/api/system/tasklogs'
...
export default {
      ...
      this.$refs.ruleForm.validate(valid => {
        if (valid) {
          if (that.form.branchNo === '0') {
            addTasklogs(that.form).then(response => {
              that.$message.success(response.msg)
              that.$emit('close', { code: response.code })
            })
          } else {
            uptTasklogs(that.form).then(response => {
              that.$message.success(response.msg)
              that.$emit('close', { code: response.code })
            })
          }
  ...
  mounted () {
    if (this.id !== '') {
      const that = this
      getTasklogs(this.id).then(response => {
