    ...
    <div>
      ...
      <vxe-table ref="myTable"
        ...
        <vxe-table-column type="seq"
                          title="序号"
                          width="60"></vxe-table-column>
        <vxe-table-column field="loginNo"
                          title="编号"
                          width="120"
                          show-overflow="tooltip"></vxe-table-column>
        <vxe-table-column field="loginName"
                          title="用户名称"></vxe-table-column>
        <vxe-table-column field="loginTime"
                          title="登录时间"></vxe-table-column>
        <vxe-table-column field="ipAddress"
                          title="IP地址"></vxe-table-column>
        <vxe-table-column field="location"
                          title="登录地点"
                          show-overflow="tooltip"></vxe-table-column>
        <vxe-table-column field="status"
                          title="登录状态"></vxe-table-column>
...
<script>
import { listLogininfo, delLogininfo, exptLogininfo } from '@/api/system/logininfo'
...
export default {
  ...
  methods: {
    ...
    doQuery () {
      ...
      if (this.queryParam.loginName !== '') {
        this.pageParam.condition = " login_name like '%" + this.queryParam.loginName + "%'"
      } else {
        ...
        this.$confirm({
          ...
          onOk () {
            ...
            selectedRecords.map(function (item) {
              selectedRowKeys.push(item.loginNo)
            })
            delLogininfo(selectedRowKeys).then(response => {
              that.getDataSource()
            })
          }
        })
    ...
    getDataSource () {
      const that = this
      this.loading = true
      listLogininfo(this.pageParam).then(response => {
