  ...
  <a-card>
    ...
    <div>
      <div class="operator">
        <a-button
          @click="handleAdd"
          v-hasPermit="['system:rotaclass:addnew']"
          type="primary"
        >新增</a-button>
        <a-button
          @click="handleDel"
          v-hasPermit="['system:rotaclass:delete']"
        >删除</a-button>
        <a-dropdown>
          <a-menu
            @click="handleMenu"
          ...
          >
            <a-menu-item
              key="audit"
              v-hasPermit="['system:rotaclass:audit']"
            >审批</a-menu-item>
            <a-menu-item
              key="import"
              v-hasPermit="['system:rotaclass:import']"
            >导入</a-menu-item>
            <a-menu-item
              key="export"
              v-hasPermit="['system:rotaclass:export']"
            >导出</a-menu-item>
          </a-menu>
          <a-button>
            更多操作
        ...
        <vxe-table-column title="操作">
          <template v-slot="{ row }">
            <vxe-button
              ...
              v-hasPermit="['system:rotaclass:update']"
            >编辑</vxe-button>
            <vxe-button
              type="text"
              @click="handleDet(row.classNo)"
              v-hasPermit="['system:rotaclass:detail']"
            >详细</vxe-button>
          </template>
        </vxe-table-column>
      </vxe-table>
...
<script>
import { listRotaclass, delRotaclass, exptRotaclass } from '@/api/system/rotaclass'
  ...
  methods: {
      ...
      if (selectedRecords.length > 0) {
        this.$confirm({
          ...
          onOk () {
            ...
            delRotaclass(selectedRowKeys).then(response => {
              that.getDataSource()
            })
          }
        })
    ...
    handleMenu (e) {
      ...
      if (e.key === 'audit') {
        ...
        exptRotaclass(this.pageParam).then(response => {
    ...
    getDataSource () {
      const that = this
      this.loading = true
      listRotaclass(this.pageParam).then(response => {
