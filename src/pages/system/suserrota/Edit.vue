...
<script>
import { getSuserrota, addSuserrota, uptSuserrota } from '@/api/system/suserrota'
...
export default {
      ...
      this.$refs.ruleForm.validate(valid => {
        if (valid) {
          if (that.form.rotaNo === '0') {
            addSuserrota(that.form).then(response => {
              that.$layer.msg(response.msg)
              that.$parent.getDataSource()
              that.$layer.close(that.layerid)
            })
          } else {
            uptSuserrota(that.form).then(response => {
              that.$layer.msg(response.msg)
              that.$parent.getDataSource()
              that.$layer.close(that.layerid)
            })
  ...
  mounted () {
    if (this.id !== '') {
      const that = this
      getSuserrota(this.id).then(response => {
