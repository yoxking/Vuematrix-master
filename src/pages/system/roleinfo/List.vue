  ...
  <a-card>
    ...
    <div>
      <div class="operator">
        <a-button
          @click="handleAdd"
          v-hasPermit="['system:roleinfo:addnew']"
          type="primary"
        >新增</a-button>
        <a-button
          @click="handleDel"
          v-hasPermit="['system:roleinfo:delete']"
        >删除</a-button>
        <a-dropdown>
          <a-menu
            @click="handleMenu"
          ...
          >
            <a-menu-item
              key="audit"
              v-hasPermit="['system:roleinfo:audit']"
            >审批</a-menu-item>
            <a-menu-item
              key="import"
              v-hasPermit="['system:roleinfo:import']"
            >导入</a-menu-item>
            <a-menu-item
              key="export"
              v-hasPermit="['system:roleinfo:export']"
            >导出</a-menu-item>
          </a-menu>
          <a-button>
            更多操作
      ...
      >
        ...
        >
          <template v-slot="{ row }">
            <vxe-button
              type="text"
              @click="allotUser(row.roleNo)"
              v-hasPermit="['system:roleinfo:allotuser']"
            >指定用户</vxe-button>
            <vxe-button
              type="text"
              @click="allotPmt(row.roleNo)"
              v-hasPermit="['system:roleinfo:allotpermit']"
            >分配权限</vxe-button>
            <vxe-button
              type="text"
              @click="handleEdt(row.roleNo)"
              v-hasPermit="['system:roleinfo:update']"
            >编辑</vxe-button>
            <vxe-button
              type="text"
              @click="handleDet(row.roleNo)"
              v-hasPermit="['system:roleinfo:detail']"
            >详细</vxe-button>
          </template>
        </vxe-table-column>
      </vxe-table>
...
export default {
  ...
  methods: {
    ...
    doQuery () {
      ...
      if (this.queryParam.roleName !== '') {
        this.pageParam.condition = " role_name like '%" + this.queryParam.roleName + "%'"
      } else {
        ...
        this.$confirm({
          ...
          onOk () {
            ...
            selectedRecords.map(function (item) {
              selectedRowKeys.push(item.roleNo)
            })
            delRoleinfo(selectedRowKeys).then(response => {
              that.getDataSource()
            })
          }
        })
    ...
    getDataSource () {
      const that = this
      this.loading = true
      listRoleinfo(this.pageParam).then(response => {
