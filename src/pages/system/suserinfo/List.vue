      ...
      <div class="operator">
        ...
        <a-dropdown>
          <a-menu @click="handleMenu"
                  slot="overlay">
            <a-menu-item key="audit" v-hasPermit="['system:suserinfo:audit']">审批</a-menu-item>
            <a-menu-item key="import" v-hasPermit="['system:suserinfo:import']">导入</a-menu-item>
            <a-menu-item key="export" v-hasPermit="['system:suserinfo:export']">导出</a-menu-item>
          </a-menu>
          <a-button>
            更多操作
            <a-icon type="down" />
        ...
        <vxe-table-column title="操作">
          <template v-slot="{ row }">
            <vxe-button type="text"
                        @click="handleEdt(row.userNo)" v-hasPermit="['system:suserinfo:update']">编辑</vxe-button>
            <vxe-button type="text"
                        @click="handleDet(row.userNo)" v-hasPermit="['system:suserinfo:detail']">详细</vxe-button>
          </template>
        </vxe-table-column>
      </vxe-table>
      <vxe-pager border
...
<script>
import { listSuserinfo, delSuserinfo, exptSuserinfo } from '@/api/system/suserinfo'
  ...
  methods: {
      ...
      if (selectedRecords.length > 0) {
        this.$confirm({
          ...
          onOk () {
            ...
            delSuserinfo(selectedRowKeys).then(response => {
              that.getDataSource()
            })
          }
        })
    ...
    handleMenu (e) {
      ...
      if (e.key === 'audit') {
        ...
        exptSuserinfo(this.pageParam).then(response => {
    ...
    getDataSource () {
      const that = this
      this.loading = true
      listSuserinfo(this.pageParam).then(response => {
