<template>
  <a-card>
    ...
    <div>
      <div class="operator">
        <a-button @click="handleAdd" v-hasPermit="['system:renteclass:addnew']" type="primary">新建</a-button>
        <a-button @click="handleDel" v-hasPermit="['system:renteclass:delete']">删除</a-button>
        <a-dropdown>
          <a-menu @click="handleMenu"
                  slot="overlay">
            <a-menu-item key="audit" v-hasPermit="['system:renteclass:audit']">审批</a-menu-item>
            <a-menu-item key="import" v-hasPermit="['system:renteclass:import']">导入</a-menu-item>
            <a-menu-item key="export" v-hasPermit="['system:renteclass:export']">导出</a-menu-item>
          </a-menu>
          <a-button>
            更多操作
            <a-icon type="down" />
      ...
      <vxe-table ref="myTable"
        ...
        <vxe-table-column title="操作">
          <template v-slot="{ row }">
            <vxe-button type="text" @click="handleEdt(row.classNo)" v-hasPermit="['system:renteclass:update']">编辑</vxe-button>
            <vxe-button type="text" @click="handleDet(row.classNo)" v-hasPermit="['system:renteclass:detail']">详细</vxe-button>
          </template>
        </vxe-table-column>
      </vxe-table>
      <vxe-pager border
...
<script>
import { listRenteclass, delRenteclass, exptRenteclass } from '@/api/system/renteclass'
  ...
  methods: {
      ...
      if (selectedRecords.length > 0) {
        this.$confirm({
          ...
          onOk () {
            ...
            delRenteclass(selectedRowKeys).then(response => {
              that.getDataSource()
            })
          }
        })
    ...
    handleMenu (e) {
      ...
      if (e.key === 'audit') {
        ...
        exptRenteclass(this.pageParam).then(response => {
    ...
    getDataSource () {
      const that = this
      this.loading = true
      listRenteclass(this.pageParam).then(response => {
